...
func generateGrpcClient(proto *model.Proto, service *model.Service) error {
	...
	for i, s := range segments {
		segments[i] = util.ToSnakeCase(s)
	}

	template, err := util.ReadFileAsString("scaffold/template/grpc_client.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	configName := util.ToPascalCase(proto.Module) + "ServiceUrl"
	...
	methodConfigTemplate, err := util.ReadFileAsString("scaffold/template/grpc_method_entry.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	...
	apiInvocationTemplate, err := util.ReadFileAsString("scaffold/template/grpc_api_invocation.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
	...
	name := util.ToSnakeCase(util.LowerFirstChar(service.Name) + "Client.go")
	err = util.WriteFileWithFormat([]byte(template), GRPC_CLIENT_PARENT_PATH+"/"+name)
	if err != nil {
...
func GenerateServiceClientRegistry(protos []*model.Proto) error {
	...
	template, err := util.ReadFileAsString("scaffold/template/service_client_registry.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}

	clientEntryTemplate, err := util.ReadFileAsString("scaffold/template/grpc_service_client_entry.tmpl")
	if err != nil {
		fmt.Println("failed to read template file:", err)
		return err
	}
